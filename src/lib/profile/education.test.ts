...
describe('parseEducationCreatePayload', () => {
  ...
  it('returns an error for invalid or missing required fields', () => {
    expect(parseEducationCreatePayload(null)).toEqual({
      error: 'Invalid request body',
    });
    expect(
      parseEducationCreatePayload({ ...validBase, institution: '' }),
    ).toEqual({ error: 'institution is required' });
    expect(
      parseEducationCreatePayload({ ...validBase, startDate: 'bad' }),
    ).toEqual({
  ...
  it('parses a valid payload with correct defaults for optional fields', () => {
    const result = parseEducationCreatePayload({
      ...validBase,
      endDate: '2022-05-31',
      gpa: '3.9/4.0',
    });
...
describe('parseEducationUpdatePayload', () => {
  it('rejects an empty update, invalid field values, and out-of-order dates when both are provided', () => {
    expect(parseEducationUpdatePayload({})).toEqual({
      error: 'No updatable fields provided',
    });
    expect(parseEducationUpdatePayload({ institution: '' })).toEqual({
      error: 'institution must not be empty',
    });
    expect(
      parseEducationUpdatePayload({
        startDate: '2022-01-01',
  ...
  it('parses a partial update and only includes provided fields', () => {
    const result = parseEducationUpdatePayload({
      degree: 'PhD',
      endDate: null,
    });
    expect(result.error).toBeUndefined();
